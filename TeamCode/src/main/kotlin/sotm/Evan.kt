...
object Evan {
    ...
    fun calculateEvan (
        ...
        val ballSpeed = sqrt(
            (distance * distance * GRAVITY) / (2 * cos(angle) * cos(angle) * (distance * tan(angle) - (targetHeight - shooterHeight)))
        )
        ...
        if (distance < 2) {
            result.targetHoodPos = KtConstants.HOOD_CLOSE
        } else {
            result.targetHoodPos = KtConstants.HOOD_FAR
        }
    }
